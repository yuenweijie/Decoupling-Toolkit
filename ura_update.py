...
# --- Sanity gate -------------------------------------------------------------
# A "successful" run that pulled no data would otherwise write a matrix of nulls
# and stamp it with today's date — a silent, invisible failure. These bounds make
# the script refuse to write rather than publish nonsense.
MIN_TRANSACTIONS = 500          # a normal 12-month window is several thousand
MIN_PRICE        =    400_000   # nothing private sells below this
MAX_PRICE        = 15_000_000   # median (not max) — a median above this is a parsing bug
...
def inject_asat(html):
    """Publish the data date as a real JS constant so the page can display it.

    The matrix dates live in HTML comments, which the page can't read at runtime
    and an external monitor can't see in the rendered output. This constant is
    what the UI renders and what the freshness monitor checks.
    """
    block = ('/* ASAT_START — auto-generated by ura_update.py */\nconst DATA_AS_AT="%s";\n/* ASAT_END */'
             % datetime.now().strftime("%Y-%m-%d"))
    new, n = re.subn(r'/\* ASAT_START.*?/\* ASAT_END \*/', lambda _: block, html, count=1, flags=re.S)
    return new, n > 0


def sanity_check(matrix, newlaunch, nused):
    """Return a list of reasons the data must not be published. Empty list = safe."""
    problems = []
    if nused < MIN_TRANSACTIONS:
        problems.append(f"only {nused} recent private transactions (expected at least {MIN_TRANSACTIONS}) "
                        "— the URA pull probably returned little or nothing")
    for label, mx in (("resale", matrix), ("new launch", newlaunch)):
        for reg in ("OCR", "RCR", "CCR"):
            for b, _, _ in BED_BANDS:
                cell = mx.get(reg, {}).get(b, {"p": None, "n": 0})
                p = cell["p"]
                if p is None:
                    problems.append(f"{label} {reg} {b}: no price (0 matching transactions)")
                elif not (MIN_PRICE <= p <= MAX_PRICE):
                    problems.append(f"{label} {reg} {b}: implausible median ${p:,}")
    return problems
...
def main():
    ...
    problems = sanity_check(matrix, newlaunch, nused)
    if problems:
        sys.exit("REFUSING TO WRITE — the data failed sanity checks:\n  - "
                 + "\n  - ".join(problems)
                 + "\n\nThe existing prices on the site are left untouched. "
                   "Check the URA access key and the batch output above.")

    html, ok1 = inject_bedmatrix(html, matrix, "MATRIX")
    html, ok2 = inject_bedmatrix(html, newlaunch, "NEWLAUNCH")
    html, ok3 = inject_asat(html)
    print("Resale matrix:", "updated" if ok1 else "MARKERS NOT FOUND",
          "| New-launch matrix:", "updated" if ok2 else "MARKERS NOT FOUND",
          "| Data-date stamp:", "updated" if ok3 else "MARKERS NOT FOUND")
    if not (ok1 and ok2 and ok3):
        sys.exit("Markers not found in Decoupling_Calculator.html — aborting without writing.")
